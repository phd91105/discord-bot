...
import { filterCommit, createEmbed } from '../utils';
...
export const cherryPick = (client: Client): void => {
  const prefix = '!pick';
  ...
  client.on(Events.MessageCreate, async (message) => {
    if (message.content.startsWith(prefix)) {
      ...
      try {
        ...
        if (commitAPI.length) {
          embedList.push(
            createEmbed(process.env.API_REPO!, commitAPI, '#ED4245'),
          );
        }

        if (commitFront.length) {
          embedList.push(
            createEmbed(process.env.FRONT_REPO!, commitFront, '#1ABC9C'),
          );
        }

        if (commitBatch.length) {
          embedList.push(
            createEmbed(process.env.BATCH_REPO!, commitBatch, '#FEE75C'),
          );
        }
